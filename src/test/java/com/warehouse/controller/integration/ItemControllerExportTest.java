...
import com.warehouse.controller.ItemController;
...
class ItemControllerExportTest extends AbstractIntegrationTest {
    ...
    void exportItems_WhenAdmin_ShouldReturnCsvStream() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(get("/api/items/export"))
                                     .andExpect(request().asyncStarted()) // Проверяем, что запустился асинхронный стриминг
                                     .andReturn();
    ...
    void exportItems_WhenUser_ShouldReturnForbidden() throws Exception {
    ...
    void exportItems_WhenAnonymous_ShouldReturnUnauthorized() throws Exception {
        mockMvc.perform(get("/api/items/export"))
