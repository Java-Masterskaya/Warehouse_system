...
### Шаг 1 — миграция `V26__add_items_barcode_nullable.sql`
...
### Шаг 1.5 — миграция `V27__create_items_barcode_seq.sql`
...
операция, деплоится вместе с V26, т.к. новый код сразу начинает вызывать
`nextval()`.
...
**Для таблиц < 100K** — копируем `V28__backfill_items_barcode.sql` в
`db/migration/` и деплоим вместе с V26/V27:
...
**Для таблиц ≥ 100K** — `V28` не трогаем вообще. Запускаем
...
### Шаг 3 — миграции V29, V30, V31

Перед деплоем проверяем не только NULL, но и дубли (иначе V31 упадёт):
...
-- V30 (требует spring.flyway.postgresql.transactional-lock: false)
...
[Старый код] ------+------+------+------+------+ [Новый код]
                    \     |      |      /
                     \    |      |     /
[V26+V27] - - - - - +----+------+----+ - - - -  (колонка nullable + sequence)
                      \   |      |   /
                       \  |      |  /
[Backfill] - - - - - - - +------+- - - - - - - -  (заполняем NULL)
                         \      /
                          \    /
[V29-V31] - - - - - - - - + - - - - - - - - - -  (NOT NULL + UNIQUE)
```

- В промежутке между V26/V27 и V29–V31 старый и новый код работают одновременно.
- V26/V27 безопасны, потому что не ломают старый INSERT.
- V29–V31 деплоятся только когда все инстансы уже новые (это отдельный деплой).
