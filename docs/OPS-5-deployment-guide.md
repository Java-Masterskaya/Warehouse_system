...
#   < 100 000  → скопировать docs/migrations/pending/V28__backfill_items_barcode.sql
#                в src/main/resources/db/migration/, задеплоить V26 + V27 + V28 + Java-код
#   >= 100 000 → НЕ копировать V28. Задеплоить только V26 + V27 + Java-код,
#                backfill — джобой (см. "Что делать, если V28 опасен" ниже)
# проверка: SELECT COUNT(*) FROM items WHERE barcode IS NULL;  → 0

# Этап 2 (отдельный деплой, когда убедились)
# проверить дубли, скопировать V29+V30+V31 из docs/migrations/pending/
...
### Этап 1: Expand + Backfill (V26 + V27 + V28)

**Что деплоится:**
- `V26__add_items_barcode_nullable.sql` — добавляет nullable колонку (уже в `db/migration/`)
- `V27__create_items_barcode_seq.sql` — независимый sequence для номера barcode,
  не привязанный к id товара (уже в `db/migration/`, см. "Почему отдельный sequence" ниже)
- `docs/migrations/pending/V28__backfill_items_barcode.sql` — UPDATE существующих строк.
...
- [ ] `SELECT COUNT(*) FROM items;` — определить, нужен ли V28 в этом деплое
...
- [ ] Если V28 не деплоился (таблица большая) — запустить backfill джобой:
...
### Этап 2: Contract (V29 + V30 + V31) — ОТДЕЛЬНО!
...
4. Backfill завершён (V28 применён либо `GET /admin/backfill/barcode/status` показывает `COMPLETE`)

**Что деплоится:**
- `V29` — `SET NOT NULL`
- `V30` — `CREATE UNIQUE INDEX CONCURRENTLY` (требует `spring.flyway.postgresql.transactional-lock: false`)
- `V31` — `ADD CONSTRAINT UNIQUE USING INDEX`

**Важно:**
- Если есть NULL-строки — V29 УПАДЁТ.
- Если есть дубли barcode — V31 УПАДЁТ. Проверяйте дубли заранее (шаг 3 выше).
- CHECK на "зарезервированный формат" больше нет — проверка живёт только в приложении.

---

## 🚨 Почему нельзя V29–V31 вместе с V26/V27/V28
...
Пользователь получит ошибку. Поэтому V29–V31 — только после полного обновления всех инстансов.

---

## 🔧 Что делать, если V28 (SQL-backfill) опасен для продакшена

Если в таблице `items` > 100 000 строк:

1. **Не копировать `V28__backfill_items_barcode.sql` в `db/migration/` вообще.**
...
5. **Деплоить V29–V31** (Этап 2 выше).

---

## ✅ Чеклист перед закрытием задачи

- [ ] V26 и V27 применены в dev/staging
- [ ] Решение по V28 принято осознанно (скопирован в `db/migration/` для маленькой
      таблицы, либо явно пропущен в пользу `ItemBarcodeBackfillJob` для большой)
- [ ] Все строки имеют barcode, дублей нет
- [ ] V29–V31 применены в dev/staging
